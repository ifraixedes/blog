...
NOTE because all the operations requires root permissions, I logged as root with `sudo su`, so all the commands in this post don't execute with `sudo`.

I decided to use [`systemd-networkd`](https://wiki.archlinux.org/title/systemd-networkd) to deal with the network configuration, and I decided to fully committed into it, so I used [`systemd-resolved`](https://wiki.archlinux.org/title/Systemd-resolved) for configuration the _network name resolution_ (a.k.a DNS). Bear in mind that my laptop only has one network interface (apart of the loopback interface).
...
I checked the DNS currently in use with `resolvectl status` and was great, it gave me the DNS servers configure on my router.
...
I open the file with the editor and wrote
...
Then, I started the service (`systemctl start iwd.service`) and confirmed that I can ping some external site.
...
Because, [I didn't want `iwd` to scan periodically for networks when it's in disconnected state](https://wiki.archlinux.org/title/Iwd#Disable_periodic_scan_for_available_networks), I created the `/etc/iwd/main.conf` file, opened with the editor, and wrote:
